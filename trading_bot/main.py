...
"""
    ...
    async def broadcast_signal(self, signal: Dict[str, Any]):
        ...
        try:
            # Log dat we beginnen met broadcasten
            logger.info(f"Starting broadcast for signal: {signal}")
            
            # Get subscribers
            subscribers = await self.db.get_subscribers(signal['instrument'])
            logger.info(f"Found {len(subscribers.data)} subscribers for {signal['instrument']}")
            ...
            sentiment_data = await self.sentiment.get_market_sentiment({
                ...
                'market': signal['market']  # Gebruik de gedetecteerde market type
            })
