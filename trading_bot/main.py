...
async def webhook(request: Request):
    ...
    try:
        ...
        if 'message' in update_data and 'text' in update_data['message'] and update_data['message']['text'].startswith('/'):
            command = update_data['message']['text'].split()[0].lower()
            logger.info(f"Commando ontvangen: {command}")
            
            # Maak een Update object
            update = Update.de_json(data=update_data, bot=telegram.bot)
            
            if command == '/start':
                logger.info("Start commando verwerken")
                await telegram.start_command(update, None)
