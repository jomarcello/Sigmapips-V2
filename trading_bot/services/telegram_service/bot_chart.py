...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def button_callback(self, update: Update, context=None) -> int:
        ...
        try:
            ...
            # Debug logging
            logger.debug(f"DEBUG: Button callback data: {callback_data}, type: {type(callback_data)}")
            ...
            if callback_data == CALLBACK_MENU_ANALYSE:
                logger.debug(f"DEBUG: Calling menu_analyse_callback with {callback_data}")
                return await self.menu_analyse_callback(update, context)
            elif callback_data == CALLBACK_MENU_SIGNALS:
                logger.debug(f"DEBUG: Calling menu_signals_callback with {callback_data}")
                return await self.menu_signals_callback(update, context)
            
            # Analysis type selection
            elif callback_data == CALLBACK_ANALYSIS_TECHNICAL or callback_data == "analysis_technical":
                logger.debug(f"DEBUG: Calling analysis_technical_callback with {callback_data}")
                ...
                logger.debug(f"DEBUG: Calling analysis_sentiment_callback with {callback_data}")
                ...
                logger.debug(f"DEBUG: Calling analysis_calendar_callback with {callback_data}")
