...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def show_sentiment_analysis(self, update: Update, context=None, instrument=None) -> int:
        ...
        try:
            ...
            # Calculate sentiment score if not properly set or if it's 0 despite having bullish/bearish data
            sentiment_score = sentiment_data.get('sentiment_score', 0)
            if sentiment_score == 0 and bullish != 50:
                # Recalculate based on bullish/bearish values
                sentiment_score = (bullish - bearish) / 100
                logger.info(f"Recalculated sentiment_score to {sentiment_score} based on bullish={bullish}, bearish={bearish}")
            ...
            # Use actual values if available, otherwise use formatted strings
            tech_score = sentiment_data.get('technical_score', 'N/A')
            if bullish > 0 or bearish > 0:
                message += f"• Technical Indicators: {tech_score}\n"
                message += f"• Bullish Sentiment: {bullish}%\n"
                message += f"• Bearish Sentiment: {bearish}%\n"
                if neutral > 0:
                    message += f"• Neutral Sentiment: {neutral}%\n"
            else:
                message += f"• Technical Indicators: {tech_score}\n"
