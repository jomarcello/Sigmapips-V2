...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def show_sentiment_analysis(self, update: Update, context=None, instrument=None) -> int:
        ...
        try:
            ...
            loading_gif = await get_loading_gif()
            loading_text = f"Analyzing sentiment for {instrument}..."
...
{self._get_trading_recommendation(bullish_score, bearish_score)}"""
            ...
            try:
                # Try to update the message with sentiment analysis
                await query.edit_message_text(
                    text=message,
                    reply_markup=InlineKeyboardMarkup(keyboard),
                    parse_mode=ParseMode.HTML
                )
                logger.info(f"Successfully displayed sentiment analysis for {instrument}")
            except Exception as text_error:
            ...
            error_text = f"❌ Could not generate sentiment analysis for {instrument}.\n\nPlease try again later."
            
            try:
                # Get the market for back button
                market = self._detect_market(instrument)
                back_data = f"market_{market}"
            except:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Could not update error message: {str(caption_error)}")
                    # Try to send a new message as fallback
                    await query.message.reply_text(
                        text=error_text,
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data=back_data)]]),
    ...
    async def show_calendar_analysis(self, update: Update, context=None, instrument=None) -> int:
        ...
        logger.info(f"Getting calendar data for {instrument}")
        ...
        try:
            ...
            try:
                # Try to update the message with calendar data
                await query.edit_message_text(
                    text=calendar_data,
                    reply_markup=InlineKeyboardMarkup(keyboard),
                    parse_mode=ParseMode.HTML
                )
                logger.info(f"Successfully displayed calendar data for {instrument}")
            except Exception as text_error:
                logger.warning(f"Could not edit message text: {str(text_error)}")
                # Try to send a new message as fallback
                await query.message.reply_text(
            ...
            error_text = f"❌ Could not generate calendar analysis for {instrument}.\n\nPlease try again later."
