...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def button_callback(self, update: Update, context=None) -> int:
        ...
        try:
            ...
            # If no specific handler is found, log it
            logger.warning(f"Unhandled callback data: {callback_data}")
            return MENU # Fallback naar hoofdmenu of huidige staat

        except BadRequest as e:
            # Handle common "Message is not modified" error silently
            if "Message is not modified" in str(e):
                logger.warning("Attempted to edit message with the same content.")
            else:
                logger.error(f"BadRequest error handling button callback: {e}")
                logger.exception(e)
            ...
            logger.error(f"Telegram error handling button callback: {e}")
            logger.exception(e)
            # Notify user if possible
            try:
            ...
            logger.error(f"Unexpected error handling button callback: {e}")
