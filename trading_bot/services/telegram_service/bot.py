...
4. No cancellation = automatic renewal at $29.99/month
    ...
    async def show_technical_analysis(self, update: Update, context=None, instrument: str = None, timeframe: str = "1h", fullscreen: bool = False) -> int:
        ...
        try:
            ...
            await query.edit_message_text(
                text=f"Generating chart for {instrument} on {timeframe} timeframe. Please wait..."
            )
            ...
            if context and hasattr(context, 'user_data'):
                ...
                context.user_data['market'] = market
                logger.info(f"Stored context for back navigation: analysis_type=technical, market={market}")
            
            # Log what we're doing
            logger.info(f"Toon technische analyse voor {instrument} op {timeframe} timeframe")
            
            try:
                # Call the chart service to get a chart
                chart_data = await self.chart.get_chart(instrument, timeframe, fullscreen)
                
                # Create an inline keyboard with timeframe options
                keyboard = []
                
                # Only add timeframe options for non-fullscreen mode
                if not fullscreen:
                    timeframe_row = []
                    for tf in ["1m", "5m", "15m", "1h", "4h", "1d"]:
                        ...
                        timeframe_row.append(
                            InlineKeyboardButton(label, callback_data=f"timeframe_{instrument}_{tf}")
                        )
                    keyboard.append(timeframe_row)
                
                # Add sentiment and calendar analysis buttons
                analysis_row = []
                sentiment_btn = InlineKeyboardButton("💭 Sentiment", callback_data=f"instrument_{instrument}_sentiment")
                calendar_btn = InlineKeyboardButton("📅 Calendar", callback_data=f"instrument_{instrument}_calendar")
                analysis_row.extend([sentiment_btn, calendar_btn])
                keyboard.append(analysis_row)
                
                # Add back button, adapted based on whether we came from a signal
                keyboard.append([
                ...
                # For standard charts, include caption with instrument and timeframe
                caption = f"📊 {instrument} ({timeframe.upper()}) Technical Analysis"
                
                # Send the chart as a photo with the keyboard
    ...
    async def analyze_from_signal_callback(self, update: Update, context=None) -> int:
        """Handle analyze market option for an instrument from a signal by showing analysis options"""
        query = update.callback_query
        await query.answer()
        
        try:
            logger.info(f"Analyze from signal for user {update.effective_user.id}")
            
            # Get callback data
            data = query.data
            parts = data.split("_")
            
            # Extract instrument from data or context
            instrument = None
            if len(parts) > 1:
                instrument = parts[-1]  # Last part should be the instrument
            ...
            if context and hasattr(context, 'user_data'):
                ...
                if instrument:
                    context.user_data['instrument'] = instrument
                    logger.info(f"Setting instrument to {instrument} from callback data")
            ...
            # Instead of directly generating a chart, show the analysis options menu
            keyboard = [
