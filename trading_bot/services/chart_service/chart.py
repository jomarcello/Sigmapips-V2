...
class ChartService:
    ...
    async def get_chart(self, instrument: str, timeframe: str = "1h", fullscreen: bool = False) -> bytes:
        ...
        try:
            ...
            max_attempts = 2  # Aantal pogingen om een screenshot te maken
            for attempt in range(max_attempts):
                if hasattr(self, 'tradingview') and self.tradingview:
                    try:
                        # Als dit niet de eerste poging is, probeer de service opnieuw te initialiseren
                        if attempt > 0:
                            logger.info("Reinitializing Node.js service for second attempt")
