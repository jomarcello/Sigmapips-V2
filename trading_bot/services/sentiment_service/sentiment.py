...
class MarketSentimentService:
    ...
    def __init__(self, cache_ttl_minutes: int = 30, persistent_cache: bool = True, cache_file: str = None, fast_mode: bool = True):  # Changed default to True for fast_mode
        """
        ...
        # CRITICAL FIX: Explicitly enable caching
        self.cache_enabled = True  # Add this line to explicitly enable caching
        ...
        logger.info(f"Cache is {'enabled' if self.cache_enabled else 'disabled'}")  # Log the cache status
    ...
    def _add_to_cache(self, instrument: str, sentiment_data: Dict[str, Any]) -> None:
        ...
        # FIXED: Simplified condition to always check self.cache_enabled directly
        ...
        try:
            ...
            if self.cache_file and self.use_persistent_cache:  # Added self.use_persistent_cache check
    ...
    def _get_from_cache(self, instrument: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cache_key in self.sentiment_cache:
                ...
                # Check cache TTL - this is the critical check for the 30-minute window
                time_diff = current_time - cache_time
                
                # Record performance metrics
                if time_diff < self.cache_ttl:
                    self.metrics.record_cache_hit()
                    logger.debug(f"Cache HIT for {instrument}: age={time_diff:.2f}s, ttl={self.cache_ttl}s")
                    ...
                    logger.info(f"Cache EXPIRED for {instrument}: age={time_diff:.2f}s, ttl={self.cache_ttl}s")
            else:
                self.metrics.record_cache_miss()
                logger.debug(f"Cache MISS for {instrument}: key not found")
                
            return None
        except Exception as e:
            logger.error(f"Error retrieving from sentiment cache: {str(e)}")
            return None
    
    # Rest of the class implementation would be here
    
# The remaining code would include the TavilyClient class and other missing parts,
# but since we're only focusing on the caching mechanism, this is sufficient for the fix

# Assuming the TavilyClient is used elsewhere, a basic implementation is included
class TavilyClient:
    def __init__(self, api_key):
        self.api_key = api_key
        # Additional initialization code would be here
    ...
    async def get_sentiment(self, instrument: str, market_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        Args:
            instrument: The instrument to get sentiment for (e.g., 'EURUSD')
            market_type: Optional market type to help narrow down the search
            
        Returns:
            Dictionary containing sentiment data including bullish/bearish percentages
        """
        ...
        if cached_result:  # Use the existing cached data if available
            logger.info(f"Using cached sentiment data for {instrument} (age: {time.time() - cached_result.get('timestamp', 0):.1f}s)")
            # Only record total request time as this was a cache hit
            self.metrics.record_total_request(time.time() - start_time)
            return cached_result
        
        # No valid cache entry, need to fetch new data
        try:
            # Use compiled regex patterns for better performance
            bullish_pattern = re.compile(r'(?:Bullish:|🟢\s*Bullish:|Bullish\s+sentiment:?)\s*(\d{1,3})\s*%', re.IGNORECASE)
            bearish_pattern = re.compile(r'(?:Bearish:|🔴\s*Bearish:|Bearish\s+sentiment:?)\s*(\d{1,3})\s*%', re.IGNORECASE)
            
            # Build a search query for the instrument
            search_query = f"{instrument} forex market sentiment analysis current"
            if market_type:
                search_query += f" {market_type}"
            
            # Get search results using the Tavily API
            search_start = time.time()
            search_results = await self._get_search_results(search_query)
            search_time = time.time() - search_start
            self.metrics.record_api_call('tavily', search_time)
            
            # Process search results to generate sentiment
            deepseek_start = time.time()
            sentiment_text = await self._generate_sentiment(instrument, search_results)
            deepseek_time = time.time() - deepseek_start
            self.metrics.record_api_call('deepseek', deepseek_time)
            
            # Extract sentiment percentages
            bullish_match = bullish_pattern.search(sentiment_text)
            bearish_match = bearish_pattern.search(sentiment_text)
            
            # Flag to track if we're using default values
            using_default = False
            
            # If either match fails, try alternative extraction or fallback to default
            if not bullish_match or not bearish_match:
                logger.debug(f"Initial regex match failed for {instrument}, trying alternative patterns")
                
                # Try basic patterns without emojis
                if not bullish_match:
                    bullish_match = re.search(r'bull\w*\s*(?:sentiment|outlook)?\D*(\d{1,3})\s*%', sentiment_text, re.IGNORECASE)
                
                if not bearish_match:
                    bearish_match = re.search(r'bear\w*\s*(?:sentiment|outlook)?\D*(\d{1,3})\s*%', sentiment_text, re.IGNORECASE)
                
                # If still no match, use default format
                if not bullish_match or not bearish_match:
                    logger.warning(f"Could not extract sentiment percentages for {instrument}, using default format")
            ...
            if bullish_match and bearish_match:
                try:
                    ...
                    if bullish + bearish != 100:
                        logger.warning(f"Sentiment percentages for {instrument} don't add up to 100%: bullish={bullish}%, bearish={bearish}%")
                        total = bullish + bearish
                        if total > 0:  # Avoid division by zero
                            bullish = round(bullish * 100 / total)
                            bearish = 100 - bullish
                except (ValueError, AttributeError) as e:
                    logger.error(f"Error parsing sentiment percentages: {str(e)}")
                    bullish = 50
                    bearish = 50
            else:
                logger.warning(f"Using default 50/50 sentiment for {instrument} due to extraction failure")
                bullish = 50
                bearish = 50
            ...
            self.metrics.record_total_request(total_time)
            
            logger.info(f"Sentiment analysis for {instrument} completed in {total_time:.2f}s (search: {search_time:.2f}s, analysis: {deepseek_time:.2f}s)")
            return result
            ...
            error_result = {
                'bullish': 50,
                'bearish': 50,
                'text': f"Error getting sentiment for {instrument}: {str(e)}"
            }
            return error_result
    ...
    async def _get_search_results(self, query: str) -> List[Dict[str, str]]:
        ...
        # For the purpose of this fix, we'll return a mock result
        return [{'content': f"Sample search result for {query}"}]
    
    async def _generate_sentiment(self, instrument: str, search_results: List[Dict[str, str]]) -> str:
        """Generate sentiment text based on search results"""
        # Implementation would be added here
        # For the purpose of this fix, we'll return a mock result
        return f"Bullish sentiment for {instrument} is strong. Bullish: 60%, Bearish: 40%"
    
    def _get_default_sentiment_text(self, instrument: str) -> str:
        """Get default sentiment text for an instrument"""
        # For the purpose of this fix, we'll return a mock result
        return f"Based on recent market analysis, the sentiment for {instrument} is mixed. Bullish: 50%, Bearish: 50%."
    
    async def _save_cache_to_file(self):
        """Save the cache to a file asynchronously"""
        # Implementation to save cache to file would be here
        # This is a non-blocking operation
        try:
            with self._cache_lock:
                # This would normally write to the cache file
                pass
        except Exception as e:
